...
- 前置された resource 自体は入力側から自動的に除外され二重投入されない (`load_sticky` +
  ...
  ため **HAPI 側では N Bundle 回検証される** (v15 実測では 17 Organization × 5,961 Bundle
  = 101k 回)
...
**実測** (2026-07-21, v15 データ、rest pass 全 25 種類 178,818 res):

| pass | resource 数 | 所要 | rps | error | fail 率 |
|---|---:|---:|---:|---:|---:|
| 通常 (`--include-file` 無し) | 178,818 | 7.6 min | 394 | **42** | 0.012% |
| `--include-file fhir_r4/Organization.ndjson` | 178,801 (−17 sticky) | **7.3 min** | 407 | **0** | **0.000%** |

- 42 errors は全て 21 Composition × 2 slice (`referralFromSection.entry:referralFromOrganization` +
  `referralToSection.entry:referralToOrganization`) 由来だったため、Organization を前置しただけで
  全部消える
- Bundle 膨張の影響は軽微 (実測 -0.3 min、むしろ rps 上昇。sticky 17 resource × 5961 Bundle =
  ~101k 追加処理相当だが、warm cache 効果が上回る)
- raw log: [`validation-results/2026-07-21_full_v15_p1000/raw/rest_with_sticky.{ndjson,meta.json,stdout.log}`](../validation-results/2026-07-21_full_v15_p1000/)
