...
日本の医療現場の FHIR データを本 validator で通す際の **特性・落とし穴・推奨構成** をまとめる。
数値例は M3 Max 14 core + Docker Desktop 18GB + 6 JVM cluster での測定
([docs/benchmarks.md](benchmarks.md) 参照)。
...
## 2. 分割検証の実測結果 (1/20 サンプル、179k res)

`/scripts/parallel-validate.py` を rest / obs 分割で回した実測例:
...
上位 issue を「原因」「対処要否」で分類。数字は 1/20 サンプル (179k res) での出現数の例。
...
- 前置された resource 自体は入力側から自動的に除外され二重投入されない (`load_sticky` +
  `bundle_generator`)。ただしこれは「未検証」を意味しない — sticky は毎 Bundle に含まれる
  ため **HAPI 側では N Bundle 回検証される** (例: 17 Organization × 6,000 Bundle = 約 10 万回)
- 前置 resource に付いた OperationOutcome issue は `filter_sticky_from_outcome` により結果
  から除去される。これは「毎 Bundle 分の重複を除去する noise 抑制」であって、sticky を
  未検証にしているわけではない
- sticky 自身の真の pass/fail を担保したい場合は、**別途 sticky ファイル単独でも 1 回検証
  する** ことを推奨:

  ```bash
  # sticky 前置版 (cross-bundle Reference 解決、fixture issue は filter)
  ./scripts/parallel-validate.py input_rest --output rest.json \
    --include-file input_rest/Organization.ndjson
  # sticky 単独版 (fixture 自身の pass/fail 確認)
  ./scripts/parallel-validate.py input_rest/Organization.ndjson --output orgs.json
  ```

- `--include-file` は複数指定可 (`Organization.ndjson`, `Location.ndjson`, ...)。file でも
  dir でも受け付ける

**実測** (rest pass 全 25 種、~180k res 規模の合成データ):

| pass | resource 数 | 所要 | rps | error |
|---|---:|---:|---:|---:|
| 通常 (`--include-file` 無し) | 178,818 | 7.6 min | 394 | 42 |
| `--include-file fhir_r4/Organization.ndjson` | 178,801 (−17 sticky) | **7.3 min** | 407 | **0** |

- 42 errors は全て Composition × 2 slice (`referralFromSection.entry:referralFromOrganization` +
  `referralToSection.entry:referralToOrganization`) 由来。Organization を前置しただけで全部消える
- Bundle 膨張の影響は軽微 (実測 -0.3 min、むしろ rps 上昇。sticky 17 resource × 約 6,000 Bundle
  = 約 10 万件の追加処理相当だが、warm cache 効果が上回る)
