...
~700ms かかり全体の律速要因になる。**Observation だけを `-tx n/a` で構造検証、他 24 種を
`-tx` 有効で完全検証** という 2 phase 分割で全体スループットが大きく改善する
(実測: 単一 pass で完走不可 → 分割で 11 分完走、v15 実測)。

### 実測 error の分類例 (v15/v17)

| error 種 | 判定した主体 | tx call 発生? |
|---|---|:---:|
| `Slice referralFromOrganization: min=1` | HAPI (slice discriminator) | ❌ |
| `Constraint failed: ele-1 (@value or children)` | HAPI (invariant) | ❌ |
| `Unknown code S67.2 in ICD-10 2019-covid-expanded` (v17) | fhirserver → HAPI に伝達 | ✅ |
| `text/plain; charset=utf-8 not in MimeType VS` (v14) | fhirserver → HAPI | ✅ |
| `無効なリソースID: 長すぎます 66文字` (v16) | HAPI (datatype rule) | ❌ |
| `SocketTimeoutException: Read timed out` (v12) | HAPI (tx 応答なし判定) | ✅ (poisoned cache) |
...
- **本 repo の default は 6 JVM** (RAM 18 GB、他プロセスと共存しやすい余裕構成)。マシン性能とバックグラウンド負荷に応じて `scripts/hapi-cluster.sh` の `PORTS` と `parallel-validate.py --ports` を増減 (8 まで安全に上げられる)
