...
- **Observation**: JP-CLINS eCS profile が `resourceIdentifier` slice を
  min=1 で要求 (system fixed は無いが `comment` で JP system 指定 = Tier 3)。
  generator (`fhir_r4_adapter.py:1414/1931`) が identifier[0] に JP system を
  必ず挿入 → **required slice `resourceIdentifier` は satisfied**、[1] は 余剰
- **Condition / AllergyIntolerance**: eCS profile は identifier に slicing block
  (discriminator=`value on system`, rules=open) を持つが **named slice 定義なし**。
  そもそも required slice が存在しないため violation 概念自体が成立しない

結論: 3 種とも **required slice は satisfied (Observation) or そもそも要求なし
(Condition/Allergy)** で、[1] の unmatched は benign 余剰。

#### (c) validator 側 quirk (47 件)

対象: `MedicationAdministration.dosage.rate.ofType(Quantity)` 47 件

R4 base MedicationAdministration profile: `dosage.rate[x]` は `type = [Ratio,
Quantity (profile=SimpleQuantity)]`、**slicing 定義なし**。data は `rateQuantity`
で SimpleQuantity 制約 (comparator 禁止) に適合しているのに HAPI が
「does not match any known slice」を出す。

これは HAPI validator が polymorphic `rate[x]` を implicit slicing とみなす
挙動由来と推定。data 側の非準拠ではない。件数が小さく (47) 独立 pattern と
しての価値は薄いため、benign 側扱いで investigation flag として記録。
...
- **Tier 2-violation** 5,440 件は JLAC 移行完了で Observation.code 分 5,032 が
  消え、残りは移行対象外 (DiagnosticReport.code 203、MedicationRequest.medication
  118、Procedure.code 44、Condition.bodySite 43) のため変動しない見込み
...
- **Tier 2-benign (c) Quantity quirk** 47 件は HAPI upstream 挙動改善で
  消える可能性がある (要 upstream 追跡)
