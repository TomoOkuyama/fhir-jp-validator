...
判定根拠 (3 種で共通、2026-07-24 patternUri 実測で確定):

- 3 種の **JP-CLINS eCS profile はいずれも `identifier:resourceIdentifier`
  slice (min=1 max=1)** を明示的に定義、かつ **`slice.system` に patternUri
  で明示 pattern**:
  - `Observation.identifier:resourceIdentifier.system` → `patternUri =
    http://jpfhir.jp/fhir/core/IdSystem/resourceInstance-identifier`
  - `Condition.identifier:resourceIdentifier.system` → 同 patternUri
  - `AllergyIntolerance.identifier:resourceIdentifier.system` → 同 patternUri
- discriminator: `value on system`、rules=open
- data の system value が上記 patternUri と一致する identifier が
  resourceIdentifier slice に match、一致しない identifier は unmatched information

**実測 (identifier 順序反転検証、2026-07-24)** — position ベースではないことの確認:

| case | identifier | 結果 |
|---|---|---|
| A: [JP-canonical, urn:internal] | 2 個 | `[1] urn:internal` unmatched information |
| B: [urn:internal, JP-canonical] | 2 個 (順序反転) | `[0] urn:internal` unmatched information |
| C: [JP-canonical] のみ | 1 個 | unmatched なし |
| D: [urn:internal] のみ | 1 個 | **error 発火**: `Slice 'Observation.identifier:resourceIdentifier': minimum required = 1, but only found 0` |

→ HAPI は spec 通り `value:system` discriminator + patternUri で pattern match
を実行しており、benign 判定は position ではなく system value に基づいている
(§3.2 の per-slice-match 原則に完全準拠)。D で slice min=1 error が発火する
ことは、resourceIdentifier slice が実効的に評価されていることを裏付ける
(fhir-jp-validator が data 非準拠を silent で見逃していない)。

- data 側の generator 処理:
  ...
  - Condition / AllergyIntolerance: 実データで identifier に JP canonical が
    含まれることを確認 (先頭固定は position 依存ではないので順序不問)

結論: **3 種とも profile 側 required slice `resourceIdentifier` (min=1) は
patternUri で pattern match、data の JP canonical identifier で satisfied**。
残りの identifier は generator 内部 ID の余剰で benign (spec 通り Open
slicing rules=open で unmatched information)。

**訂正メモ**:
- v1 (2026-07-24 初稿) 「Cond/Allergy は profile 側 named slice なし」は
  grep 条件バグで slice sub-element を拾い損ねた誤り、v2 で 3 種とも slice
  定義あり と訂正
- v2 「system は Fixed/Pattern なし (Tier 3 の prose only)」は `patternUri`
  field の見落とし。v3 (本節) で `patternUri` 明示定義済 (Tier 1 相当の
  spec 符号化) と訂正、識別子順序反転検証で挙動確認
